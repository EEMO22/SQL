...
SELECT rgn.region_name 지역이름, cntr.country_name 나라이름
FROM regions rgn JOIN countries cntr
                ON cntr.region_id = rgn.region_id
ORDER BY rgn.region_name ASC, cntr.country_name DESC;
...
SELECT cntr.country_name 나라명, cntr.country_id 나라아이디,
    loc.city 도시명, loc.location_id 도시아이디,
    dept.department_name 부서명,
    dept.department_id 부서아이디
FROM countries cntr INNER JOIN locations loc
...
SELECT emp.employee_id 사번, emp.first_name||' '||last_name 이름, 
    emp.job_id 업무아이디,
    jh.start_date 시작일, jh.end_date 종료일
FROM employees emp JOIN job_history jh
                     ON emp.job_id = jh.job_id AND jh.job_id = 'AC_ACCOUNT';
...
SELECT dept.department_id 부서번호, dept.department_name 부서이름,
    emp.first_name 매니저이름, loc.city "위치한 도시", cntr.country_name "나라의 이름",
    rgn.region_name "지역구분의 이름"
