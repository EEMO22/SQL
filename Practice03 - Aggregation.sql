...
SELECT department_id 부서, ROUND(AVG(salary), 2) 평균임금, MIN(salary) 최저임금, 
    ROUND((AVG(salary) - MIN(salary)), 2) "평균임금 - 최저임금"
FROM employees
GROUP BY department_id
...
SELECT jobs.job_title 업무, (MAX(salary) - MIN(salary)) "최고임금과 최저임금의 차이"
...
SELECT manager_id 관리자, ROUND(AVG(salary)) 평균급여,
...
SELECT first_name 이름, hire_date 입사일,
    CASE  WHEN hire_date < '2002-12-31'
                        THEN '창립멤버'
                     WHEN hire_date BETWEEN '2003-01-01' AND '2003-12-31'
                        THEN '03년입사'
                     WHEN hire_date BETWEEN '2004-01-01' AND '2004-12-31' 
                        THEN '04년입사'
                     ELSE '상장이후입사'
    END optDate
FROM employees
